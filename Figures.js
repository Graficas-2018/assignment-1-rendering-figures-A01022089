...
function createSquare(gl)
{
    ...
    //give the context you recieve, verts you choose for the figure, the primitive to draw
    var square = createFigure(gl, verts, gl.TRIANGLE_STRIP);
    //return the figure
    return square;
}
...
function createTriangle(gl)
{
    ...
    //give the context you recieve, verts you choose for the figure, the primitive to draw
    var triangle = createFigure(gl, verts,gl.TRIANGLES);
    //return the figure
    return triangle;
}
...
function createRhombus(gl)
{
    ...
    //give the context you recieve, verts you choose for the figure, the primitive to draw
    var rhombus = createFigure(gl, verts, gl.TRIANGLE_STRIP);
    //return the figure
    return rhombus;
}
...
function createSphere(gl, radius)
{
    ...
    for(var i = 40.0; i < 310.0; i++)
    {
      //convert the angle from degrees to radians
      var degToRad = i * Math.PI / 180;

      //get the new point coordinates and apply the radius size
      var point = [radius * Math.cos(degToRad), radius * Math.sin(degToRad), 0.0]

      //add the new point to the array with the others
      verts = verts.concat(point);
    }

    //give the context you recieve, verts you choose for the figure, the primitive to draw
    var sphere = createFigure(gl, verts, gl.TRIANGLE_FAN);
    //return the figure
    return sphere;
}

//less code in the other figures
function createFigure(gl, verts, type)
{
  var vertexBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
  //vertSize always 3 because we work in three dimensions
  figure = {buffer:vertexBuffer, vertSize:3, nVerts:verts.length/3, primtype:type};

  //return the figure ready to be drawn
  return figure;
}
